...
namespace DotnetBleServer.Gatt
{
    public class GattApplicationManager
    {
        ...
        public async Task RegisterGattApplication(IEnumerable<GattServiceDescription> gattServiceDescriptions, string device)
        {
            ...
            await RegisterApplicationInBluez(applicationObjectPath, device);
        }
        ...
        private async Task RegisterApplicationInBluez(string applicationObjectPath, string device)
        {
            var gattManager = _ServerContext.Connection.CreateProxy<IGattManager1>("org.bluez", device);
